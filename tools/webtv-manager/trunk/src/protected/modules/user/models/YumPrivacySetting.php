...
class YumPrivacysetting extends YumActiveRecord{
	...
	{
		if(isset(Yum::module()->privacySettingTable))
			$this->_tableName = Yum::module()->privacySettingTable;
		elseif(isset(Yii::app()->modules['user']['privacySettingTable']))
			$this->_tableName = Yii::app()->modules['user']['privacySettingTable'];
		else
			$this->_tableName = 'wtvmT_PrivacySetting'; // fallback if nothing is set
	...
	public function rules()
	{
		return array(
			...
			array('appear_in_search, message_new_friendship, message_new_message, message_new_profilecomment', 'numerical', 'integerOnly'=>true),
